...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)ctx {
    ...
    if (animateSlide) {
        ...
        if (self.isPresenting) {
            ...
            fromFrame.origin.y = container.bounds.size.height/4;
        }
        else {
            toFrame.origin.y   = container.bounds.size.height;
            fromFrame.origin.y = - container.bounds.size.height/4;
        ...
        //[self transitionDuration:ctx]
        [UIView animateWithDuration:1.5f delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:5 options:kNilOptions animations:^{
